...
def play_game(game_in_func, start='blue'):
    ...
    for row in game_in_func.iterrows():
        row = row[1]
        if row['label'] == 'black':
            game_in_func_dictionary[row['word']] = 0 # 2
        elif row['label'] == 'red':
            game_in_func_dictionary[row['word']] = 1 # 0
        elif row['label'] == 'blue':
            game_in_func_dictionary[row['word']] = 2 # 0
        elif row['label'] == 'grey':
            game_in_func_dictionary[row['word']] = 3 # 2
    ...
    while((len(game_in_func.loc[game_in_func['label'] == 'red']) != 0) or (len(game_in_func.loc[game_in_func['label'] == 'blue']) != 0)):
        ...
        for g in guess:
            if g == game_in_func.loc[game_in_func['label'] == 'black'].values[0]:
